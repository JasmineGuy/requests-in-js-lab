...
sayHelloButton.addEventListener("mouseover", setBackgroundColor);
...
const ohMy = () => {
    ...
    .then(res => {
        for(let i= 0; i <res.data.length; i++) {
            document.createElement("p").textContent ="res.data[i]"
        }
    })
}
...
const repeatMyParam = (event) => {
    axios.get(`http://localhost:3000/repeat/${event.target.id}`)
    .then(res => {
...
// CODE HERE
const someQuery = () => {
    axios.get('http://localhost:3000/query-test?name=jasmine)')
    .then(res) => {
